    ...
    class ChartReview extends BindingClass {
        constructor() {
            ...
            this.bindClassMethods([
                'mount',
                'searchNotificationsByReminderType',
                'searchMedicationByStatus',
                'searchNotificationByCurrentTime',
        ...
        async mount() {
            ...
            document.getElementById('retrieve-notifications-type-form').addEventListener('click', this.searchNotificationsByReminderType); // 添加事件监听，当按钮被点击时调用 getNotificationsByReminderType 方法
           document.getElementById('retrieve-medications-status-form').addEventListener('click', this.searchMedicationByStatus); // Changed to call searchMedications instead of getMedicationsByMedicationStatus
           document.getElementById('retrieve-notifications-current-time-form').addEventListener('click', this.searchNotificationByCurrentTime);
        ...
        async searchNotificationsByReminderType(event) {
                event.preventDefault();

                const searchButton = document.getElementById('retrieve-notifications-type-form');
                const origButtonText = searchButton.innerText;
                searchButton.innerText = 'Searching...';

                try {
                    await this.getNotificationsByReminderType();
                } catch (error) {
                    console.error('Error searching medications by type:', error);
                } finally {
                    searchButton.innerText = origButtonText;
                }
            }

        async searchMedicationByStatus(event) {
                event.preventDefault();

                const searchButton = document.getElementById('retrieve-medications-status-form');
                const origButtonText = searchButton.innerText;
                searchButton.innerText = 'Searching...';

                try {
                    await this.getMedicationsByMedicationStatus();
                } catch (error) {
                    console.error('Error searching medications by status:', error);
                } finally {
                    searchButton.innerText = origButtonText;
                }
            }
        async searchNotificationByCurrentTime(event) {
                       event.preventDefault();

                       const searchButton = document.getElementById('retrieve-notifications-current-time-form');
                       const origButtonText = searchButton.innerText;
                       searchButton.innerText = 'Searching...';

                       try {
                           await this.getNotificationByCurrentTime();
                       } catch (error) {
                           console.error('Error searching medications by time:', error);
                       } finally {
                           searchButton.innerText = origButtonText;
                       }
                   }
